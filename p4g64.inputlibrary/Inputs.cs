...
namespace p4gpc.inputlibrary
{
    public unsafe class Inputs : IInputHook
    {
        ...
        private long _baseAddress; // Base address (probably won't ever change)
        ...
        public Inputs(IReloadedHooks hooks, Config configuration, Logging utils, long baseAddress, IMemory memory)
        {
            ...
            try
            {
                ...
                hookSigScans.Add(Task.Run(() => keyboardAddress = SigScan("85 DB 74 05 E8 ?? ?? ?? ?? 8B 7D F8", "keyboard hook")));
                hookSigScans.Add(Task.Run(() => controllerAddress = SigScan("0F AB D3 89 5D C8", "controller hook")));
